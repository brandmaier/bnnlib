...
ln <-Sequence_size(seq1)
  ...
  inp <- rep(NA, ln)
  for (i in 1:ln) {
		outp <- Network_get_output(network,i)
...
library(ggplot2)
  ...
  p1 <- ggplot(data=melted, aes(x=Var1,y=value,group=Var2,group=Var2,color=Var2))+geom_point()+geom_line()+
    theme_minimal()+ theme(legend.position="bottom")+ ylab("Value")+xlab("Time")+ ggtitle("True Value")

 
  melted <- reshape2::melt(trt)
     melted$Var2 <- factor(melted$Var2)  
  p2<- ggplot(data=melted, aes(x=Var1,y=value,group=Var2,group=Var2,color=Var2))+geom_point()+geom_line()+
