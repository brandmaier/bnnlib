...
Trainer_train2(trainer, sequence_set, 200)
#Trainer_add_abort_criterion(trainer, ConvergenceCriterion(1e-5))
...
plotPredictions(network, sequence)
```
...
```{r}
plotActivations(network, sequence) + theme_minimal()

plotActivations(network, sequence,"CEC") + theme_minimal()
```