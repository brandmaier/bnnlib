...
num_hidden = 16
sparsity = 0.1
net <- NetworkFactory_createSparseAutoencoder(dims,
                                              num_hidden,
                                              sparsity, 
                                              bnnlib::TANH_NODE, 
                                              bnnlib::SIGMOID_NODE)
...
Trainer_sparsity_beta_set(trainer,10)
 
Sys.time()
Trainer_train__SWIG_0(trainer, sequence_set, 25)
Sys.time()
```
...
weights <- getOutgoingWeights(e1,e2)

war <- simplify2array(weights)
hist(war)
```